# -*- coding: utf-8 -*-
"""
Created on Tue Apr 17 07:26:25 2018

@author: vikkas
"""
# for testing run time at different node combinations
...
#    nx.draw(G,with_labels = True, with_edges=True)
    ...
    print("A graph with nodes = ",n," and edges = ",m," has been downloaded to file with the name test.weighted.edgelist as arc matrics")
    return G
...
def read_file():
    ...
    path = 'c:/py_prac/test.weighted.edgelist'
...
import pandas as pd
df = pd.DataFrame(columns=['Number_nodes','Number_edges','Edge ratio','Method','Time taken'])
...
for j in range (1,21,1):
    ...
    # storing it as list 
    ratio=(K.number_of_edges()/(K.number_of_nodes()*(K.number_of_nodes()-1)))
    import time
    start_time = time.time()
    ...
    df.loc[len(df)]=[i,K.number_of_edges(),ratio,method_g,time_taken]
